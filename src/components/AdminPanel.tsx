...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [taskCooldownSecs, setTaskCooldownSecs] = useState(5);
  ...
  const applySettingsData = (data: any) => {
    ...
    setTaskCooldownSecs(data.taskCooldownSecs ?? 5);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const settingsData = {
        ...
        taskCooldownSecs: taskCooldownSecs || 5,
      };
                ...
                <div className="bg-[#0d0d10] rounded-[16px] p-4 border border-blue-500/20 space-y-3 mt-2">
                  ...
                  {taskEnabled && (
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1.5">Ad এর পরে Cooldown (seconds)</label>
                        <div className="flex items-center gap-3 bg-[#1a1a1d] rounded-[12px] px-4 py-3 border border-blue-500/20">
                          <button onClick={() => setTaskCooldownSecs(Math.max(0, taskCooldownSecs - 1))} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">−</button>
                          <input type="number" value={taskCooldownSecs} onChange={e => setTaskCooldownSecs(Number(e.target.value))}
                            className="flex-1 bg-transparent text-blue-300 font-black text-xl outline-none text-center" />
                          <button onClick={() => setTaskCooldownSecs(taskCooldownSecs + 1)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">+</button>
                        </div>
                        <p className="text-zinc-600 text-[10px] mt-1.5">প্রতিটা Ad শেষ হওয়ার পর {taskCooldownSecs}s wait করতে হবে। Movie/Episode Ad এও এই সময় লাগবে।</p>
                      </div>
                      <div className="bg-blue-500/5 border border-blue-500/15 rounded-[12px] p-3">
                        <p className="text-zinc-500 text-[10px]">ℹ️ Ad Network: Adsgram চালু থাকলে Adsgram, না থাকলে Monetag চলবে।</p>
                      </div>
                    </div>
                  )}
                </div>
...
const TasksPanel: React.FC = () => {
  ...
  // Form state
  const [title, setTitle]       = useState('');
  const [rewardPerAd, setRewardPerAd] = useState(3);
  const [rewardType, setRewardType]   = useState<'taka'|'coin'>('taka');
  const [maxAdsPerDay, setMaxAdsPerDay] = useState(10);
  const [order, setOrder]       = useState(1);
  const [enabled, setEnabled]   = useState(true);

  const showT = (msg: string) => { setToast(msg); setTimeout(() => setToast(''), 3000); };

  const resetForm = () => { setTitle(''); setRewardPerAd(3); setRewardType('taka'); setMaxAdsPerDay(10); setOrder(tasks.length+1); setEnabled(true); setEditId(null); };
  ...
  const handleSave = async () => {
    if (!title.trim()) { showT('❌ Title দাও'); return; }
    setSaving(true);
    try {
      const data = { title: title.trim(), rewardPerAd, rewardType, maxAdsPerDay, order, enabled, updatedAt: serverTimestamp() };
      if (editId) {
  ...
  const handleEdit = (t: any) => {
    setEditId(t.id); setTitle(t.title||''); setRewardPerAd(t.rewardPerAd??3);
    setRewardType(t.rewardType??'taka'); setMaxAdsPerDay(t.maxAdsPerDay??10);
    setOrder(t.order??1); setEnabled(t.enabled!==false);
  };
  ...
  const handleToggle = async (t: any) => {
    await updateDoc(doc(db,'tasks',t.id), { enabled: !t.enabled });
  };

  return (
    <div className="space-y-4 py-2">
      {toast && (
        <div className="fixed top-[70px] left-4 right-4 z-[70] bg-emerald-500/20 text-emerald-50 border border-emerald-500/30 px-4 py-3 rounded-2xl text-sm font-bold shadow-xl">
          {toast}
        </div>
      )}

      <h2 className="text-lg font-black text-white">📋 Tasks Management</h2>

      {/* Note about ad system */}
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-[16px] p-3">
        <p className="text-blue-300 text-xs font-bold">ℹ️ Ad System: Settings এ Adsgram চালু থাকলে Adsgram চলবে, না থাকলে Monetag চলবে।</p>
      </div>

      {/* Add/Edit Form */}
      <div className="bg-[#1a1a1d] border border-white/5 rounded-[20px] p-4 space-y-3">
        <h3 className="text-sm font-black text-white">{editId ? '✏️ Task Edit' : '➕ নতুন Task'}</h3>

        <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Task title (যেমন: ভিডিও এড দেখে আয়🤑)"
          className="w-full px-4 py-3 bg-[#0d0d10] border border-white/8 rounded-[14px] text-white text-sm outline-none focus:border-blue-500/40" />

        {/* Reward type */}
        <div className="flex gap-2">
          {(['taka','coin'] as const).map(t => (
            <button key={t} onClick={()=>setRewardType(t)}
              className={`flex-1 py-2.5 rounded-[12px] text-xs font-black transition-all ${rewardType===t?'bg-blue-500/20 text-blue-300 border border-blue-500/30':'bg-white/5 text-zinc-500'}`}>
              {t==='taka'?'৳ Taka':'🪙 Coin'}
            </button>
          ))}
        </div>
        ...
        {/* Reward per ad */}
        <div>
          <label className="text-xs text-zinc-500 mb-1.5 block">প্রতি Ad এ reward ({rewardType==='taka'?'৳':'Coin'})</label>
          <div className="flex items-center gap-3 bg-[#0d0d10] rounded-[12px] px-4 py-3 border border-white/8">
            <button onClick={()=>setRewardPerAd(Math.max(1,rewardPerAd-1))} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400">−</button>
            <input type="number" value={rewardPerAd} onChange={e=>setRewardPerAd(Number(e.target.value))}
              className="flex-1 bg-transparent text-white font-black text-xl outline-none text-center" />
            <button onClick={()=>setRewardPerAd(rewardPerAd+1)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400">+</button>
          </div>
        </div>

        {/* Max ads per day */}
        <div>
          <label className="text-xs text-zinc-500 mb-1.5 block">দিনে সর্বোচ্চ কতটা Ad</label>
          <div className="flex items-center gap-3 bg-[#0d0d10] rounded-[12px] px-4 py-3 border border-white/8">
            <button onClick={()=>setMaxAdsPerDay(Math.max(1,maxAdsPerDay-1))} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400">−</button>
            <input type="number" value={maxAdsPerDay} onChange={e=>setMaxAdsPerDay(Number(e.target.value))}
              className="flex-1 bg-transparent text-white font-black text-xl outline-none text-center" />
            <button onClick={()=>setMaxAdsPerDay(maxAdsPerDay+1)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400">+</button>
          </div>
          <p className="text-zinc-600 text-[10px] mt-1">মোট আয় = {rewardType==='taka'?'৳':'🪙'}{maxAdsPerDay*rewardPerAd} / দিন</p>
        </div>
        ...
        <div className="flex gap-3 items-center">
          <div className="flex-1">
            ...
            <input type="number" value={order} onChange={e=>setOrder(Number(e.target.value))}
              className="w-full px-3 py-2.5 bg-[#0d0d10] border border-white/8 rounded-[12px] text-white text-sm outline-none" />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-zinc-500">চালু</span>
            <button onClick={()=>setEnabled(!enabled)}
              className={`relative w-10 h-5 rounded-full transition-all ${enabled?'bg-emerald-500':'bg-white/10'}`}>
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} disabled={saving}
            className="flex-1 bg-blue-500 text-white py-3 rounded-[14px] font-black text-sm flex items-center justify-center gap-2">
            <Save size={15} />{saving?'Saving...':(editId?'Update':'Add Task')}
          </button>
          {editId && (
            <button onClick={resetForm} className="px-4 py-3 bg-white/5 text-zinc-400 rounded-[14px] font-bold text-sm">
              Cancel
            </button>
          )}
        </div>
      ...
      {/* Task List */}
      {loading ? (
        <div className="flex justify-center py-8"><div className="w-7 h-7 border-2 border-blue-500/20 border-t-blue-500 rounded-full animate-spin" /></div>
      ) : tasks.length === 0 ? (
        <div className="text-center py-8 text-zinc-500 text-sm">কোনো Task নেই। উপরে Add করো।</div>
      ) : (
        <div className="space-y-2">
          {tasks.map(t => (
            <div key={t.id} className="bg-[#1a1a1d] border border-white/5 rounded-[18px] p-4 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className={`font-bold text-sm ${t.enabled===false?'text-zinc-500':'text-white'}`}>{t.title}</p>
                <p className="text-zinc-500 text-[11px] mt-0.5">
                  {t.rewardType==='coin'?'🪙':'৳'}{t.rewardPerAd} / Ad · দিনে {t.maxAdsPerDay} টা · মোট {t.rewardType==='coin'?'🪙':'৳'}{(t.rewardPerAd||0)*(t.maxAdsPerDay||0)}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={()=>handleToggle(t)}
                  className={`relative w-9 h-5 rounded-full transition-all ${t.enabled!==false?'bg-emerald-500':'bg-white/10'}`}>
                  <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${t.enabled!==false?'left-4':'left-0.5'}`} />
                </button>
                <button onClick={()=>handleEdit(t)} className="p-2 bg-blue-500/15 text-blue-400 rounded-[10px]"><Edit size={14} /></button>
                <button onClick={()=>handleDelete(t.id)} className="p-2 bg-red-500/15 text-red-400 rounded-[10px]"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
